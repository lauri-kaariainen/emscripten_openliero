...
void Gfx::loadMenus()
{
    ...
    //EMSCRIPTEN CHANGE: let's make the choises unselectable (because you can't change them)
	MenuItem tempItem = MenuItem(48, 7, "NAME", PlayerMenu::PlName);
	tempItem.selectable = false;
	playerMenu.addItem(tempItem);
	...
	tempItem = MenuItem(48, 7, "AIM UP", PlayerMenu::PlUp);
	tempItem.selectable = false;
	playerMenu.addItem(tempItem);
	
	tempItem = MenuItem(48, 7, "AIM DOWN", PlayerMenu::PlDown);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "MOVE LEFT", PlayerMenu::PlLeft);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "MOVE RIGHT", PlayerMenu::PlRight);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "FIRE", PlayerMenu::PlFire);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "CHANGE", PlayerMenu::PlChange);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "JUMP", PlayerMenu::PlJump);tempItem.selectable = false;playerMenu.addItem(tempItem);
	tempItem = MenuItem(48, 7, "DIG", PlayerMenu::PlDig);tempItem.selectable = false;playerMenu.addItem(tempItem);
...
void Gfx::flip()
{
	...
	//emscripten wants surface to be locked to display anything on canvas
	SDL_LockSurface(back);
	//std::cout << "gfx:flip() surface locked" << std::endl;
		if(mag != prevMag)
		{
			...
			//std::cout << "gfx:flip() surface unlocked" << std::endl;
	...
	//uncommenting these will give you constant fps-stream in console
...
void Gfx::EMSCRIPTENselectSTBANA2Map()
{
	settings->randomLevel = false;
	settings->regenerateLevel = true;
	//settings->levelFile.clear();
	settings->levelFile = "STBANA2.LEV";
	return;
}
...
{
	settings->wormSettings[0]->loadProfile("Profiles/GoofyLeft.lpf");
	return;
}

//EMSCRIPTEN-edit
void Gfx::EMSCRIPTENselectProfileForPlayer2()
{
	settings->wormSettings[1]->loadProfile("Profiles/GoofyRight.lpf");
	return;
}
...
void Gfx::selectProfile(WormSettings& ws)
{
	...
	//do all the rest in main loop because emscripten:
	globalLoopStatus.loopStatusVarPrevious = globalLoopStatus.loopStatusVar;
	globalLoopStatus.wormSettingsWeAreHandling = &ws;
	globalLoopStatus.loopStatusVar = "select_profile_menu";
...
void Gfx::playerSettings(int player)
{
	...
	globalLoopStatus.wormSettingsWeAreHandling = playerMenu.ws.get();
...
int Gfx::mainLoop(loopStatusStruct* loopStatus){
//std::cout << "mainloop called with " << loopStatus->loopStatusVar << " and selection: " << loopStatus->menuselect <<std::endl;

	if(loopStatus->loopStatusVar.compare("initial") == 0){
		...
		EMSCRIPTENselectProfileForPlayer1();
		EMSCRIPTENselectProfileForPlayer2();
		...
		SDL_LockSurface(back);
		//std::cout << "gfx::mainloop_init surface locked" << std::endl;
		controller->draw(*this);
		
		SDL_UnlockSurface(back);
		//std::cout << "gfx:mainloop_init() surface unlocked" << std::endl;
		...
		//emscripten wants surface to be locked to display anything on canvas
		SDL_LockSurface(back);
		//std::cout << "gfx:menuloop() surface locked" << std::endl;
		...
		//emscripten wants surface to be locked to display anything on canvas
		SDL_UnlockSurface(back);
		//std::cout << "gfx:menuloop() surface unlocked" << std::endl;
	...
	if(loopStatus->loopStatusVar.compare("menuloop_iter") == 0){
...

		{
			...
			|| testSDLKeyOnce(SDLK_KP_ENTER))
			{
				if(curMenu == &mainMenu)
				{
					...
					switch (s)
					{
						...
						{
							loopStatus->menuselect = curMenu->onEnter(common);
							loopStatus->loopStatusVar = "mainloop_continue_after_menuloop";
							break;
						}
			...
			if(testSDLKeyOnce(SDLK_F10))
			{
				curMenu = &mainMenu;
				loopStatus->loopStatusVar = "mainloop_continue_after_menuloop";
				loopStatus->menuselect = MainMenu::MaQuit;
			}
			...
			// Remember to unlock..
			SDL_UnlockSurface(back);
			//std::cout << "gfx:menuloop() do-while surface unlocked" << std::endl;
			menuFlip();
	...
	if(loopStatus->loopStatusVar.compare("mainloop_continue_after_menuloop") == 0){
		...
		if(loopStatus->menuselect == MainMenu::MaNewGame)
		{
		...
		else if(loopStatus->menuselect == MainMenu::MaResumeGame)
		{
		
		}
		else if(loopStatus->menuselect == MainMenu::MaQuit) // QUIT TO OS
		{
			//break;
			
		}
		else if(loopStatus->menuselect == MainMenu::MaReplay)
		{
	...
	//selectProfile
	if(loopStatus->loopStatusVar.compare("select_profile_menu") == 0){
		FileSelector profileSel(*common, 28);
		//emscripten wants surfaces to be locked to draw to canvas
		SDL_LockSurface(back);
		//std::cout << "locking surface in selectProfile" <<std::endl;
		{
		...
		profileSel.currentNode->getMenu().moveTo(loopStatus->profileMenuSelect);
		profileSel.draw();

		if (!profileSel.process()){
			//return to previous step
			loopStatus->loopStatusVar = loopStatus->loopStatusVarPrevious;
		...
		|| testSDLKeyOnce(SDLK_KP_ENTER))
		{
			...
			if (sel)
			{
				...
				loopStatus->wormSettingsWeAreHandling->loadProfile(sel->fullPath);
				//return to previous step
				loopStatus->loopStatusVar = loopStatus->loopStatusVarPrevious;

				//run playermenu.updateItems here to refresh changes after load (won't happen on emscripten-version otherwise)
				playerMenu.updateItems(*common);
				playerMenu.moveToFirstVisible();
		...
		//emscripten wants surfaces to be locked to draw to canvas
		SDL_UnlockSurface(back);
		//std::cout << "UNlocking surface in selectProfile2" <<std::endl;
	}
